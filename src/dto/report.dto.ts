...
export const createRefundReportSchema = yup.object({
  orderId: yup
    .string()
    .test('is-objectId', 'Mã đơn hàng không hợp lệ', (val) => isObjectId(val || ''))
    .required('orderId là bắt buộc'),
  
  paymentId: yup
    .string()
    .test('is-objectId', 'Mã thanh toán không hợp lệ', (val) => isObjectId(val || ''))
    .required('paymentId là bắt buộc'),
  
  // Các trường đã làm phẳng (Flat)
  cusName: yup
    .string()
    .required('Tên khách hàng (cusName) là bắt buộc'),
  ...
  refundBy: yup
    .string()
    .required('Người thực hiện hoàn tiền là bắt buộc'),
